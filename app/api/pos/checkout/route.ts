...
export async function POST(request: Request) {
  try {
    ...
    for (const item of items) {
      ...
      const variantData: Record<string, any> = variant || {};
      const productTitle = Array.isArray(variantData.Product) 
        ? (variantData.Product[0]?.title || "Item") 
        : (variantData.Product?.title || "Item");

      if (variant.currentStockLevel < quantity) {
        return NextResponse.json(
          { error: `Insufficient stock for ${productTitle} (${variant.size || ""} ${variant.color || ""}). Available: ${variant.currentStockLevel}, Requested: ${quantity}` },
          { status: 400 }
        );
      }
      ...
      lineItemsToInsert.push({
        variantId,
        sku: variant.sku,
        title: `${productTitle} (${variant.size || ""} / ${variant.color || ""})`,
        quantity,
        price: itemPrice
      });
