  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        {/* POs Directory */}
        <div className={`lg:col-span-2 space-y-4 ${selectedPO ? "hidden lg:block" : ""}`}>
        ...
        {selectedPO ? (
          <div className="lg:col-span-1 space-y-4">
            <Card className="border border-indigo-100 shadow-lg bg-white relative overflow-hidden">
              <div className="absolute top-0 left-0 w-full h-1 bg-indigo-500"></div>
              
              <CardHeader className="p-4 border-b border-slate-100 flex flex-row items-center justify-between bg-indigo-50/20">
                <div>
                  <button 
                    onClick={() => setSelectedPO(null)}
                    className="flex items-center gap-1 text-[11px] font-bold text-indigo-600 uppercase tracking-wider mb-1 hover:underline lg:hidden"
                  >
              ...
              <CardContent className="p-4 space-y-5">
                ...
                <div className="grid grid-cols-2 gap-3 text-xs border-b border-slate-100 pb-4">
                  <div className="space-y-1">
                    <span className="text-slate-400 font-medium flex items-center gap-1"><User className="w-3.5 h-3.5 text-slate-400" /> Vendor</span>
                    <span className="font-semibold text-slate-800 block">{selectedPO.vendorName}</span>
                    {selectedPO.vendorEmail && (
                      <span className="text-[10px] text-slate-500 font-mono flex items-center gap-1"><Mail className="w-3 h-3 text-slate-400" /> {selectedPO.vendorEmail}</span>
                    )}
                  </div>
                  <div className="space-y-1">
                    <span className="text-slate-400 font-medium flex items-center gap-1"><Building className="w-3.5 h-3.5 text-slate-400" /> Destination</span>
                    <span className="font-semibold text-slate-800 block">{selectedPO.warehouse?.name}</span>
                    <span className="text-[10px] text-slate-400 font-mono">Code: {selectedPO.warehouse?.code}</span>
                  </div>
                </div>
                ...
                <div className="space-y-3">
                  ...
                  <div className="space-y-2 max-h-[40vh] overflow-y-auto pr-1">
                    {selectedPO.items?.map((item) => {
                      ...
                      return (
                        <div key={item.id} className="p-3 bg-slate-50 rounded-lg border border-slate-100 flex flex-col justify-between gap-2">
                          <div className="flex justify-between items-start">
                            <div>
                              ...
                              <span className="text-[10px] bg-slate-200/80 text-slate-600 font-medium px-1.5 py-0.5 rounded mt-1 inline-block mr-1">
                                Size: {item.variant?.size}
                              </span>
                              <span className="text-[10px] bg-slate-200/80 text-slate-600 font-medium px-1.5 py-0.5 rounded mt-1 inline-block">
                          ...
                          <div className="flex items-center justify-between border-t border-slate-100 pt-2 mt-1">
                            ...
                            {selectedPO.status !== "COMPLETED" && selectedPO.status !== "CANCELLED" && (
                              <div className="flex items-center gap-1">
                                <label className="text-[10px] font-bold text-slate-400 mr-1">Inward:</label>
                                <input
                                  type="number"
                                  min="0"
                                  ...
                                  className="w-16 bg-white border border-slate-200 rounded-md p-1 text-xs text-center font-mono focus:outline-none focus:ring-1 focus:ring-indigo-500"
                                />
                              </div>
                            )}
                          </div>
                  ...
                  {selectedPO.status !== "COMPLETED" && selectedPO.status !== "CANCELLED" ? (
                    <>
                      <Button
                        ...
                        className="w-full bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-xs flex items-center justify-center gap-1.5"
                      >
                        {isReceiving ? (
                          <>
                      ...
                      {selectedPO.status === "SENT" && (
                        <Button
                          ...
                          variant="outline"
                          className="w-full border-rose-200 text-rose-600 hover:bg-rose-50 text-xs font-semibold flex items-center justify-center gap-1.5"
                        >
                          <Trash2 className="w-3.5 h-3.5" /> Cancel Purchase Order
                        </Button>
                      )}
                    </>
                  ) : (
          ...
          <div className="hidden lg:block lg:col-span-1">
            <Card className="border border-dashed border-slate-200 h-96 flex items-center justify-center text-center p-6 bg-slate-50/50">
              <div>
                <div className="w-12 h-12 rounded-full bg-slate-100 flex items-center justify-center text-slate-400 mx-auto mb-3">
                  <FileText className="w-6 h-6" />
                </div>
                <h3 className="font-bold text-slate-800 text-sm">No PO Selected</h3>
                <p className="text-xs text-slate-400 mt-1 max-w-xs mx-auto">
                  Select a purchase order from the directory ledger list to view items and receive stock shipment.
                </p>
              </div>
            </Card>
          </div>
