...
export async function getContextSession(): Promise<ContextSession> {
  ...
  // If not found and code is "syn" or "seyon", try the alternative code
  if (!company) {
    const alternativeCode = code === "syn" ? "seyon" : (code === "seyon" ? "syn" : null);
    if (alternativeCode) {
  ...
  // Final fallback: fetch the first available company in the DB
  if (!company) {
